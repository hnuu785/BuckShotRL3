...
4. 아이템 시스템 및 세부 규칙
아이템은 새로운 탄창이 장전될 때 지급됩니다 (인당 2~4개 추가). 이전 라운드의 아이템은 유지됩니다. 인벤토리 제한은 8슬롯입니다. 새 아이템 추가 시 인벤토리 제한을 초과하면, 제한(8슬롯)에 도달할 때까지만 추가됩니다.
...
장전 단계:
실탄/공포탄 조합을 무작위 생성하여 개수를 표시합니다.
각 라운드 시작 시 플레이어에게 2~4개의 랜덤 아이템을 추가합니다. 이전 라운드의 아이템은 유지됩니다. 새 아이템 추가 시 인벤토리 제한(8슬롯)을 초과하면, 제한에 도달할 때까지만 추가됩니다.
참고: 장전 단계에서 체력(HP)은 회복되지 않습니다. 체력은 게임 초기화 시에만 리셋됩니다.
참고: 장전 단계에서 아이템은 제거되지 않습니다. 모든 아이템은 라운드 간에 유지됩니다.

액션 단계:
수갑 등 상태 이상을 확인합니다.
플레이어의 입력을 기다립니다 (아이템 사용, 상대 사격, 자신 사격).
사격 결과를 처리하여 체력을 갱신하고 턴 전환을 관리합니다.

조건 확인:
탄창이 비면: 장전 단계로 전환 (새 라운드 시작, 체력은 회복되지 않음).
체력이 0 이하가 되면: 게임 종료 (게임 종료이지 라운드 종료가 아님).
그 외: 이전 액션 결과와 상태 이상에 따라 다음 차례를 결정합니다.
...
7. 라운드 정의
라운드(Round)는 한 세트의 총알이 모두 소진될 때까지의 기간을 의미합니다. 참고: 플레이어의 체력이 0이 되면 게임이 종료됩니다 (라운드 종료가 아님).

라운드 시작 (Load Phase):
- 새 총알 세트 생성: 1~4개의 실탄과 1~4개의 공포탄을 무작위로 생성하여 총 2~8개의 총알을 만듭니다.
- 총알 섞기: 생성된 총알들을 무작위로 섞어 산탄총에 장전합니다.
- 아이템 배분: 각 라운드 시작 시 플레이어에게 2~4개의 랜덤 아이템을 추가합니다. 이전 라운드의 아이템은 유지됩니다. 새 아이템 추가 시 인벤토리 제한(8슬롯)을 초과하면, 제한에 도달할 때까지만 추가됩니다.
- 총알 정보 공개: 플레이어에게 실탄과 공포탄의 개수를 표시합니다 (예: "실탄 3발, 공포탄 2발").
- **중요**: 체력(HP)은 회복되지 않습니다. 체력은 이전 라운드의 상태를 유지하며 게임 시작 시에만 초기화됩니다.
- **중요**: 아이템은 제거되지 않습니다. 이전 라운드의 모든 아이템이 유지됩니다.
...
라운드 종료 조건:
1. 탄창 소진: 모든 총알이 발사되거나 배출되어 탄창이 비어있을 때 (rounds.Count == 0) → 새 라운드(Load Phase)가 즉시 시작됩니다.
2. 체력 소진: 플레이어 또는 딜러 중 한 명의 체력이 0이 될 때 → **게임 종료** (라운드 종료가 아님).

라운드 종료 시:
- 남은 총알이 있다면 모두 제거됩니다.
- **중요**: 아이템은 제거되지 않습니다. 모든 아이템은 라운드 간에 유지되며 다음 라운드로 이어집니다.
- 즉시 새로운 라운드(Load Phase)가 시작되어 새로운 총알 세트가 생성됩니다.
...
- 체력(HP)은 라운드 간에 유지되며 게임 초기화 시에만 리셋됩니다. 새 라운드가 시작될 때 체력은 회복되지 않습니다.
